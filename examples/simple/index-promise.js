const dgraph = require('../../lib/index')

const ADDR = process.env.DGRAPH_URL
    ? process.env.DGRAPH_URL
    : 'http://localhost:8080'
const AUTH = {
    'eq-api-jwt': process.env.JWT ? process.env.JWT : '',
}

// Create a client stub.
function newClientStub() {
    // return new dgraph.DgraphClientStub('http://localhost:8080')
    return new dgraph.DgraphClientStub(ADDR, AUTH)
}
...
// show status
function showStatus(dgraphClient) {
    return dgraphClient.health()
}
...
function main() {
    ...
    return showStatus(dgraphClientStub)
